...
class CookiesMiddleware(RetryMiddleware):
    ...
    def process_request(self, request, spider):
        redisKeys = self.redis.keys()
        while len(redisKeys) > 0:
            elem = random.choice(redisKeys)
            if "%s:Cookies" % spider.name in elem:
                cookie = json.loads(self.redis.get(elem))
    ...
    def process_response(self, request, response, spider):
        ...
        #if response.status in [300, 301, 303]:
            try:
                ...
                if "login.weibo" in redirect_url or "login.sina" in redirect_url:  # Cookie失效
                    ...
                    updateCookie(request.meta['accountText'], self.redis, spider.name)
                elif "weibo.cn/security" in redirect_url:  # 账号被限
                    logger.warning("One Account is locked! Remove it!")
                    removeCookie(request.meta["accountText"], self.redis, spider.name)
