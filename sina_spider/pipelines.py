...
from items import UserItem, TweetsItem, RelationshipsItem
...
class ESPipeline(object):
    def __init__(self):
        #self._client = Elasticsearch(['http://10.134.35.52:9200'])
        self._client = Elasticsearch(['http://10.134.13.99:9200'])

    def process_item(self, item, spider):
        if isinstance(item, TweetsItem):
            dic = dict(item)
            docid = dic['id']
            self._client.index(index='pyspider', doc_type='tweets', body=dic, id=docid)
        elif isinstance(item, UserItem):
            dic = dict(item)
            docid = dic['id']
            self._client.index(index='pyspider', doc_type='feeds', body=dic, id=docid)
        return item
